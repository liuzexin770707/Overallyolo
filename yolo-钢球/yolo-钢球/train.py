...
# ================= 1. 训练参数自定义配置区 =================
MODEL_WEIGHTS = 'yolo26n.pt'            # 预训练模型，首次运行会自动下载。可选 yolov8s.pt, yolov8m.pt 等
DATA_YAML = './yolo_dataset/dataset.yaml' # 刚才脚本1生成的 yaml 文件路径
EPOCHS = 200                            # 训练轮数 (建议初测用50-100，正式用300)
BATCH_SIZE = 8                         # 批次大小 (如果显存爆了报错 CUDA out of memory，请将其调小为 8 或 4)
IMG_SIZE = 320                          # 输入图片大小
PROJECT_NAME = 'yolo_runs'              # 训练结果保存的主文件夹名
RUN_NAME = 'train_exp_1'                # 本次训练结果的子文件夹名
WORKERS = 0                             # Windows + OpenCV 4.12 兼容性问题，改为 0 禁用多进程加载
